...
- **Flag A/B** (`--flag aa|scaling|fog|floorfast|floorhalf`): interleaves baseline/flagged runs A,B,A,B (defeats thermal drift), temporarily flipping the compile-time const in-source with a guarded git-restore. `fog` and `floorfast` are inverted (their defaults are on; the flagged variant measures turning each *off*).
