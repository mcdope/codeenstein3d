...
- **The calibration figure below is not a throughput number, and reading it as one costs hours.** Measured 2026-08-04 on the same Ryzen 5800X: a *full 17-level* Gamer/hard run at `CONCURRENCY=10` sustains **0.31 attempts/minute — about 3.2 minutes per attempt**, with the machine CPU-saturated (load 14.9 of 16 threads, ~990% CPU). Raising concurrency does not help; it is already the bottleneck. The 5m13s figure below looks four times faster only because those 8 attempts were racing to *qualify* (clear level 4) and mostly died early rather than playing all 17 levels — and because 8 attempts at `CONCURRENCY=8` all fit in one wave, where 60 attempts take six. Budget a fixed-denominator sweep at **~3.2 min/attempt**, i.e. roughly 3.2 hours per 60-attempt combo, and expect easy/normal to be *slower* than hard because the bot survives deeper. A 9-combo × 60-attempt sweep is ~29 hours, not the ~8 the figure below suggests.
