...
### Two real findings from this tool's own smoke testing

**A mine-corridor stall.** A uniform-Casual 2-player pair reproducibly hits the same `stall`/`healthDrainFrozen` anomaly sequence, at near-identical tick counts and positions, across separate independent attempts — around a mined corridor on `demo-campaign/main.c` (~pos `(37.5, 49.3)`, mine distance ~3.0-4.0 tiles).

**A far more severe, likely multiplayer-specific stall.** The same runs also hit one much longer stall — ~596 ticks (vs. ~40-45 for the mine-corridor one above), at a *different* position (~pos `(15.8, 16.8)`), with **no mine and no threat nearby** (`threatDist=none mineDist=none`). Reproduced at the same position across every attempt of `npm run balancing:scan-multiplayer`'s own default preset (3/3 attempts, 0/1 qualifying — this preset's documented "0 qualifying" result *is* this finding, not a tool bug). No mine/threat involved rules out the mine-corridor explanation — a bot-vs-bot navigation conflict (two bots occupying/contesting the same tile or corridor) is the leading theory, since single-player's own bot never has another bot to contend with at all. Unconfirmed without further investigation.

Both are exactly the kind of regression this tool exists to surface — flagged here for a future balancing pass, not fixed as part of shipping the tool itself.
