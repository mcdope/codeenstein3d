...
Both campaign orchestrators (`balancing:campaign`, `balancing:campaign-multiplayer`) can spread their local lanes across N SSH hosts as well, on top of (not instead of) `CODEENSTEIN_CAMPAIGN_LANES`/`_MP_CAMPAIGN_LANES` local lanes — useful when one machine's own core count is the bottleneck, or (for the multiplayer campaign specifically) when running more than one *local* lane isn't possible at all (see below).
