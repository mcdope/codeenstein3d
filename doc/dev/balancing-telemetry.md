...
**Check the guards *can* fail before trusting that they didn't (2026-08-11).** The four-arm aim A/B ran the recipe above exactly as written and every arm cleared **20/20 at every level with a 0.000 death rate** — so `qualifyRate` and the conditional death rate could not have breached their thresholds whatever the change did, and "GUARDS: pass" carried no information at all. This is the demo campaign's own easiness (levels 1-14 clear at ~99% on Hard) showing up as a measurement limitation, not a fluke of that change. At `LEVEL_LIMIT=8` on this campaign the protocol currently validates **win metrics only**. Before reading a guard pass as evidence, confirm the baseline side actually dies somewhere; if it doesn't, either raise `LEVEL_LIMIT` past the levels that kill (15 is the wall) or stage a repo that does — a guard with a zero base rate is the same broken experiment as an A/B whose control never exhibits the phenomenon (see [Matched-scale verification](#matched-scale-verification) above).
