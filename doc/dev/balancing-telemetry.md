...
**A mine-corridor stall (open, not fixed).** A uniform-Casual 2-player pair reproducibly hits the same `stall`/`healthDrainFrozen` anomaly sequence, at near-identical tick counts and positions, across separate independent attempts — around a mined corridor on `demo-campaign/main.c` (~pos `(37.5, 49.3)`, mine distance ~3.0-4.0 tiles). Exactly the kind of regression this tool exists to surface — flagged here for a future bot-behavior balancing pass, not fixed as part of shipping the tool itself.
