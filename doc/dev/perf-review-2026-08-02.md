...
>
> But **"`strokeRect` is expensive" was also wrong**, and it took a third round to land on the real rule. A controlled injection on the now-clean frame cannot reproduce any cost from `strokeRect` at all: 1, 2, 6, 12 or 24 of them per frame, small or large, integer or fractional coordinates, integer or fractional line width, early or late in the op stream, in one run or interleaved with hundreds of `fillRect`s between each — every one of those measures free. What is *not* free is a `strokeRect` drawn while **`lineJoin` is `"round"`**: **48.0fps at one call, 47.9 at six**, against 59.0 with the default miter join. `drawWeapon` sets `ctx.lineJoin = "round"` for the whole weapon (`viewmodel.ts:91`), which is exactly why its outlines cost the frame — and why, in the confirming experiment where `outlineRect` was temporarily reverted to `ctx.strokeRect`, the one scenario that stayed fast was **automap-open, the single state in which the viewmodel is not drawn at all**.
>
> So the class is not "outlines" and not "non-rectangular shapes" — it is **geometry Skia cannot emit as axis-aligned quads**. A rect outline with miter joins decomposes into four quads; the same outline with round joins does not. See §6 for what that reclassifies.
...
**Still stroked at this point, converted in the two follow-up commits below:** the three toasts and the lore overlay (`hud.ts:86/117/147/300`), and the weapon-drop and teleporter rings (`sprites.ts:653/815`).
...
#### Outcome — toasts, lore overlay, and the `lineJoin` root cause, 2026-08-02

The last four `strokeRect`s (`hud.ts` — the cheat, out-of-ammo and acid-overflow toasts, and the lore overlay box) went to `outlineRect`, finishing the sweep. Chasing whether `strokeText` needed the same treatment is what finally produced the correct mechanism (see the correction at the top and the reclassified entries in §6): **the trigger is `lineJoin = "round"`, not `strokeRect`.**

That also exposed a real inconsistency this work had introduced and nothing had caught: `drawWeapon` sets `lineJoin = "round"` on the scene context, but a `Glyph` pre-renders on a *detached* context that inherits none of it — so every baked weapon polygon had silently switched from round joins to miter. The five stroking weapon glyphs now set the join themselves.

Final measurement, all four scenarios, after everything:

| scenario | original | final | dropped |
|---|---|---|---|
| idle | 51.3 fps | **59.1** | 7 / 473 |
| combat | 48.8 fps | **58.0** | 15 / 464 |
| automap open | 44.3 fps | **58.6** | 11 / 469 |
| automap closed | 47.9 fps | **59.6** | 3 / 477 |
...
- **`strokeRect` with the default miter join.** Free — the original synthetic finding was right, for the wrong reason, and survived two attempts to overturn it. Injected on the clean frame at 1/2/3/4/6/8/12/24 per frame, small (80×20) and large (84×200), integer and fractional coordinates, `lineWidth` 1 and 1.5, early and late in the op stream, and interleaved with 300 `fillRect`s between each: **every cell measures 58.0–59.5 fps against a 58.5–59.2 baseline.** The HUD health-bar border, the minimap panel frame and the toast/lore boxes were never the problem.
- **`strokeRect` with `lineJoin = "round"`.** **NOT free — this is the real second half of the bug.** One per frame: **48.0 fps**. Three: 48.2. Six: 47.9. Against 59.0 with `lineJoin = "miter"` and the same six calls. The penalty is the same fixed, count-independent, position-independent one a path fill carries, because a round-joined rect outline is not decomposable into axis-aligned quads. `drawWeapon` sets `lineJoin = "round"` (`viewmodel.ts:91`) around every weapon it draws, so all fourteen of the viewmodel's outlines were paying it.
- **`strokeText`.** Free. 1, 2 and 8 per frame on the clean render path: 58.2 / 58.5 / 59.3 fps against a 58.5–59.2 baseline, indistinguishable from the same count of `fillText`. The kill-streak banner, the exit countdown and the spectating banner need no change.
- **Sub-pixel placement.** Not a trigger for anything. Rounding every coordinate *and* the line width to whole pixels in situ moved nothing (47.5 fps against a 49.0 baseline, on the still-broken build).

  **Lesson for the next audit, and it cost three rounds to learn:** a synthetic replica only tests the variables you thought to vary. The first replica cleared `strokeRect` because it happened to use the default join; the in-situ revert then indicted `strokeRect` wholesale because the only real call sites left were inside a `lineJoin = "round"` block. Neither the "free" nor the "expensive" verdict was about `strokeRect`. The way out was the same one that found the bug originally — **subtract from the real thing and look at which scenario disagrees**: automap-open staying fast while everything else broke is what pointed at the viewmodel, and from there at the join.
