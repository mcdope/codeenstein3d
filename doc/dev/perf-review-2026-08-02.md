...
Every per-frame **non-rectangular canvas path fill or path stroke** — `beginPath`/`moveTo`/`lineTo`/`arc` followed by `fill()` or `stroke()` — forces Chrome's GPU-process canvas rasteriser out of its batched quad path and into a coverage/stencil pass. On an accelerated 2D canvas that already carries the raycaster's ~2,600 quads plus a full-canvas `putImageData`, that pass misses the vsync deadline. The penalty is **fixed per frame and all-or-nothing**: one path fill costs the same as sixty-four.
...
- **`strokeRect`.** Free. 0 → 64 `strokeRect`s per frame on top of the full bulk workload: 60.0 / 59.3 / 59.7 / 59.3 / 59.7 / 58.7 / 59.7 fps. Axis-aligned rect strokes take Skia's fast path. The HUD health-bar border (`hud.ts:463`), the minimap panel frame (`raycaster.ts:653`) and the viewmodel's `strokeRect`s can stay. *Measured in the synthetic harness only — confirm in situ during Phase 3.*
