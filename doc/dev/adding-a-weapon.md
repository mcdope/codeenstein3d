...
| `scripts/lib/loadEngineModules.mjs` | the `loot.ts` re-export list — a hand-maintained *string of names*, so a constant missing from it is `undefined` in every harness script rather than a build error |
| `scripts/lib/levelSolver.mjs` | one line in `dropAmountsFrom`'s `constantFor` map. It throws on a pool it cannot price, and `levelSolver.test.mjs` pins the set against the real weight tables, so this one fails loudly — which is new: see below |

**The two `scripts/` rows are there because `shells` fell straight through this checklist.** They were added on 2026-08-21, after the fact. When the shotgun got its own pool on 2026-08-15, neither the harness re-export list nor the solver's drop-amount map gained a `shells` entry, and nothing failed: `dropAmounts.shells` was `undefined`, so the offline solver's drop budget went `NaN`, every `clearRatio.combined` went `NaN` with it, and — because **`NaN < 1` is `false`** — the solver's own "not clearable" guard stopped being able to fire at all, on every repo, for six days. `stage-campaign.mjs` picks the level a staged campaign must include with `reduce((a, b) => ratio(b) < ratio(a) ? b : a)`, which under `NaN` always keeps `a`, so "include the tightest level" quietly became "include whichever level came first". The map is derived and throws now, and the checklist names the files; both were silent before.
