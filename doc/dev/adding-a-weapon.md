...
- `meleeRange` vs `maxRange` — both are hard distance caps in `resolveShot` (`rangeLimit = weapon.meleeRange ?? weapon.maxRange`), but `meleeRange` additionally *makes the weapon melee* everywhere else. Don't reach for it to mean "short-ranged gun"; that's `maxRange` (only Friday Hotfix uses it).
