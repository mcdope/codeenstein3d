...
| Highscore board (`codeenstein-highscores`) | Score, campaign/level name, levels cleared, your Player name if you set one, a SHA-256 hash of the *whole workspace's* parsed code structure (used only to tell "same code" runs apart, regardless of which level a run ended on — not reversible into source), lines-of-code/complexity totals, and — if captured — a replay |
...
| Preferences | Player name, gore level, difficulty, render quality, and Master/SFX/Music volume (`codeenstein-player-name` and friends) — simple settings, not tied to any run |
