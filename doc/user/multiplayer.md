...
- **Loot is shared.** Anything an Elite or regular enemy drops is visible to the whole team on the minimap and automap, not just whoever's standing next to it — once someone has been near that part of the level. That last bit is the one thing either map still keeps back, and it exists so a player dropping out of the session doesn't instantly mark their exact position for everyone.
- **You can see each other on the maps.** Every teammate is a dot on your minimap and automap, in the same colour their name floats in above them in the world — anywhere on the level, including parts nobody has walked yet.
- **You can call for help.** Press `G` (or `Y` on a pad) and your dot lights up on everyone else's map for about five seconds: it brightens, a sonar ring sweeps out of it, a siren sounds, and a banner under their minimap names you. The marker follows you while it lasts, so it still points at you if you're running. There's roughly an eight-second wait before you can call again.
