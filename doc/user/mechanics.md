...
**You also get the ping without walking into anything.** Pass a room with a key still sitting in it — one you can actually reach — and it pings once, the same marker and sonar ring, without the thunk or the banner, because nothing refused you. It fires once per key per level: the point is to make sure you noticed it, not to nag you about a detour you already decided against. A key lying loose in a corridor pings too, once you are within a few tiles of it.
