...
- **Want a simple grunt?** A short, flat function with low cyclomatic complexity (few branches/loops). HP is complexity × 35.
...
- **Want a locked room?** Make a method private or protected. Its pack also comes out *guarded* — a tougher gatekeeper in front, lighter escorts behind, same total strength. The generator always places a reachable key somewhere else in the level first — you can't lock yourself out by writing this. A key is coloured, permanent, and opens every door of its room, so one key is all a room ever costs. A level locks at most **four rooms** — one per key colour, so a colour never repeats within a level — picking the ones most worth locking (large, loot-rich, few ways in), so in a file full of private methods only some of them become locked rooms; otherwise a big source file turned into nothing but a key hunt.
...
- **Want extra supplies right at spawn?** More top-level imports/`#include`s — roughly one alcove per four, capped at four. They stock ammo for weapons you already own, so early levels lean toward bullets and shells.
