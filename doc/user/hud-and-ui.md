...
- **Gore** — None / Normal / More / Extreme / Excessive / Absurd, controls blood-particle volume and how long stains linger. **Excessive** is Extreme with about a third more blood on screen at once, lingering about a third longer. **Absurd** is a different thing rather than more of the same: blood that has landed never dries up, so a room you fought through stays painted for the rest of the level, and the splats on the floor are bigger. It is capped — the oldest blood makes way once there is enough of it — but it is the most expensive setting by a wide margin, so try one tier down first if the game feels choppy.
