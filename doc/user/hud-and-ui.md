...
Press `Tab` to open the full automap, which pauses the game. It's fog-of-war — only tiles you've actually visited are drawn — and shows walls, secret walls (with a very slight tint difference from real ones), lore terminals, doors, teleporters, traps (color-coded active/safe), hazard tiles, any mines you've already spotted, and the exit once found.
