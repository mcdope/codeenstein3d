...
An always-on minimap sits in the corner, showing the full layout of the current level — walls, doors, hazard tiles, spike traps (color-coded active/safe), teleporters, uncollected keys, lore terminals, and the exit — from the moment the level loads, with **no fog-of-war**: nothing is gated on where you've actually walked. (The Tab automap below works the same way — it used to be the exception, and no longer is.) Your own position and facing are shown as a bright triangle, mines and enemies only appear once you've actually spotted them, and the exit marker is always visible, not revealed later. A small circular compass badge is attached to its bottom-right corner — its needle points toward the exit relative to whichever way you're currently facing (dead ahead reads as "up").
...
- **Difficulty** — Easy / Normal / Hard, scales enemy HP, enemy damage, pickup scarcity, and how many rollbacks a run gets (2 / 1 / none). **It locks as soon as a run starts** and opens again when the run ends — the highscore board records which difficulty a run was played on, and that is only meaningful if it held for the whole run. Gore and render quality stay changeable, since neither touches the simulation or your score. See [Mechanics](mechanics.md#difficulty) for exact numbers.
- **Render quality** — Classic or Sharp. Classic keeps the game's intended chunky retro look (640×400) and is the default. Sharp renders internally at 1280×800 — crisper walls, sprites, weapon, status bar and maps — paired with a half-resolution floor so it stays fast, and is worth trying on a high-refresh (120Hz+) display. Everything is the same *size* in both modes and sits in the same place; Sharp simply draws it with twice as many pixels. Like gore, a change applies at the next level you launch (a small note in the panel says so), and aiming and shots behave identically in both modes — the quality is purely visual.
- **Automap orientation** — *Rotate with you* (the default) turns the Tab automap so your facing is always up; *Fixed (north up)* keeps it still, which is easier if you're trying to memorise a level's shape rather than navigate it right now. Unlike gore and render quality, this one takes effect immediately.
