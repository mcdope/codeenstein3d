...
describe("threat give-up", () => {
  const T = { ...DEFAULT_TUNING, THREAT_GIVEUP_TICKS: 5, THREAT_GIVEUP_COOLDOWN_TICKS: 20 };
  const near = (over = {}) => makeEnemy({ x: 12.5, y: 10.5, alive: true, aggroed: true, hp: 100, maxHp: 100, ...over });

  it("stops picking a threat the fight is not advancing against", () => {
    const memory = { abandoned: new Set() };
    const enemies = [near()];
    // Drive `decide` past the threshold with a threat it can neither hit nor
    // be hit by — a wall between them, so no fire and no HP change.
    for (let i = 0; i < T.THREAT_GIVEUP_TICKS + 2; i++) {
      decide({ player: makePlayer(), enemies, mines: [], map: undefined, navTarget: { x: 40, y: 40 }, projectiles: [] },
        memory, { profile: PROFILE, tuning: T, stepMs: 50 });
    }
    expect(memory.threatCooldown.get(0), "the enemy should be on cooldown").toBeGreaterThan(memory.decisionTicks);
    expect(pickThreat(enemies, makePlayer(), PROFILE, undefined, T, memory)).toBeUndefined();
  });

  it("re-engages once the cooldown expires — a required kill must stay possible", () => {
    // Deliberately not the mine's permanent abandon: `exitRoomHasAliveEnemy`
    // gates the exit on its room's enemies being dead, so refusing forever
    // would trade a wedge for an unclearable level.
    const memory = { abandoned: new Set(), threatCooldown: new Map([[0, 10]]), decisionTicks: 11 };
    expect(pickThreat([near()], makePlayer(), PROFILE, undefined, T, memory)).toBeDefined();
  });

  it("does NOT give up while the fight is advancing", () => {
    // The false-positive that would matter most: giving up on something the
    // bot is actually killing.
    const memory = { abandoned: new Set() };
    const enemies = [near()];
    for (let i = 0; i < T.THREAT_GIVEUP_TICKS * 3; i++) {
      enemies[0].hp -= 1; // it is losing health, so the fight is progressing
      decide({ player: makePlayer({ x: 11.0 }), enemies, mines: [], map: undefined, navTarget: { x: 40, y: 40 }, projectiles: [] },
        memory, { profile: PROFILE, tuning: T, stepMs: 50 });
    }
    expect(memory.threatCooldown.get(0) ?? 0).toBe(0);
  });

  it("counts a stall that MOVES, which is what the old counter could not", () => {
    // `combatStallTicks` resets on any position change, so the stall-strafe it
    // triggers resets it. A jittering bot must still be recognised as stuck.
    const memory = { abandoned: new Set() };
    const enemies = [near()];
    for (let i = 0; i < T.THREAT_GIVEUP_TICKS + 2; i++) {
      const x = 10.5 + (i % 2) * 0.1; // the exact 0.1-tile jitter seen in the wedges
      decide({ player: makePlayer({ x }), enemies, mines: [], map: undefined, navTarget: { x: 40, y: 40 }, projectiles: [] },
        memory, { profile: PROFILE, tuning: T, stepMs: 50 });
    }
    expect(memory.threatCooldown.get(0), "jitter must not mask the stall").toBeGreaterThan(0);
  });

  it("tracks the enemy's own index, not its position in the candidate list", () => {
    // With a dead enemy ahead of it, a positional index would put the cooldown
    // on the wrong enemy entirely.
    const memory = { abandoned: new Set(), threatCooldown: new Map([[1, 999]]), decisionTicks: 0 };
    const enemies = [makeEnemy({ alive: false }), near()];
    expect(pickThreat(enemies, makePlayer(), PROFILE, undefined, T, memory)).toBeUndefined();
  });
});
