...
/**
 * Fingerprint of the tuning every profile ships with.
 *
 * `src/engine/defaultHighscore.ts` is generated by playing the campaign with
 * these profiles, so it stops representing the current bot the moment any of
 * them changes — and **nothing else detects that**. The per-level `astHash`
 * covers the parsed source and `balanceHash` covers the enemy roster; neither
 * knows anything about bot tuning, so every replay stays valid while the board
 * quietly describes a bot that no longer exists. That is not hypothetical:
 * the shipped file has gone stale this way more than once.
 *
 * The generator bakes this value into the file it writes, and
 * `profiles.test.mjs` recomputes it here and compares — turning an invisible
 * drift into a failing `vitest run`.
 *
 * Keys are sorted before hashing so that reordering fields, which changes no
 * behaviour, does not read as a change. Values are compared exactly, so any
 * real retune does.
 */
export function profilesHash() {
  const stable = (value) => {
  ...
  // Profile *order* is deliberately preserved: several consumers read the
  // ladder positionally (`curateMixedProfiles` treats key order as tier
  // order), so a reordering is a real change even though a field reorder
  // inside one profile is not.
  const ordered = LADDER_ORDER.map((name) => [name, stable(PROFILES[name])]);
  return createHash("sha256").update(JSON.stringify(ordered)).digest("hex");
}

/** Ladder order, weakest first — see `profilesHash`. */
const LADDER_ORDER = Object.keys(PROFILES);
