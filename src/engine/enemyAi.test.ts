...
describe("updateEnemies", () => {
  it("skips a dead enemy entirely", () => {
    ...
    const damage = updateEnemies([e], player, map, 1, [], pathFieldFor(map, player));
  ...
  it("cools down attackCooldown and fireCooldown by dt, floored at 0", () => {
    ...
    updateEnemies([e], player, map, 1, [], pathFieldFor(map, player));
  ...
  it("wakes up (aggroes) once the player is within range and visible", () => {
    ...
    updateEnemies([e], player, map, 0.016, [], pathFieldFor(map, player));
  ...
  it("roams toward its target when not aggroed", () => {
    ...
    updateEnemies([e], player, map, 0.1, [], pathFieldFor(map, player), noRng);
  ...
  it("picks a new roam target once it arrives at the current one", () => {
    ...
    updateEnemies([e], player, map, 0.1, [], pathFieldFor(map, player), () => 0.5);
  ...
  it("picks a new roam target when a wall blocks the stroll", () => {
    ...
    expect(() => updateEnemies([e], player, map, 0.5, [], pathFieldFor(map, player), () => 0.5)).not.toThrow();
  });
  ...
  it("an Edge Case enemy can abandon its roam target early via the retarget roll", () => {
    ...
    updateEnemies([e], player, map, 0.1, [], pathFieldFor(map, player), () => 0);
  ...
  it("attacks in melee range when the cooldown has elapsed", () => {
    ...
    const damage = updateEnemies([e], player, map, 0.016, [], pathFieldFor(map, player));
  ...
  it("fires a ranged bolt at a visible target within range, and sets a new cooldown", () => {
    ...
    updateEnemies([e], player, map, 0.016, projectiles, pathFieldFor(map, player), () => 0.5);
  ...
  it("does not fire while its own fire cooldown is still active", () => {
    ...
    const projectiles: Projectile[] = [];
    updateEnemies([e], player, map, 0.016, projectiles, pathFieldFor(map, player));
  ...
  it("moves toward the player while chasing at range", () => {
    ...
    const e = enemy({ x: 5, y: 5, aggroed: true });
    const before = { x: e.x, y: e.y };
    updateEnemies([e], player, map, 0.1, [], pathFieldFor(map, player));
    expect(e.x).not.toBe(before.x);
  });
  ...
  it("an Edge Case chases noticeably faster than a regular enemy", () => {
    ...
    updateEnemies([regular], player, map, 0.1, [], pathFieldFor(map, player));
    updateEnemies([edge], player, map, 0.1, [], pathFieldFor(map, player));
  ...
  it("aggregates damage across multiple enemies in one call", () => {
    ...
    const damage = updateEnemies([a, b], player, map, 0.016, [], pathFieldFor(map, player));
    expect(damage).toBe(20);
  });
  ...
  it("falls back to straight-line steering when the player's tile is outside the pathing window", () => {
    ...
    const e = enemy({ x: 5, y: 5, aggroed: true });
    const before = { x: e.x, y: e.y };
    expect(() => updateEnemies([e], player, map, 0.1, [], pathFieldFor(map, player))).not.toThrow();
