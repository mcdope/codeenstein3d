...
Clearing a level scores it — kill value, remaining health/ammo, completion
speed, and route efficiency, minus a malus for finishing hurt — and records a
top-10 entry (score, campaign name, and a SHA-256 hash of the level's AST +
campaign name, for comparing runs) in a sidebar Highscores dialog. A sidebar
Master/SFX/Music volume mixer balances the procedural sound effects against
an optional custom BGM folder (local `.mp3`/`.ogg`/`.wav` files, played back
as a shuffled, looping playlist).
...
* **Highscores** – a sidebar button opens a dialog listing the top-10 scored runs (score, campaign name, and a truncated SHA-256 hash of the level's AST + campaign name, so you can tell whether a leaderboard entry is really the same code)
