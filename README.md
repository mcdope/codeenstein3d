...
detected entrypoint (or the first parsable file if none is found), dropping
you straight into the dungeon generated from its structure. Fight through to
the `return` statement — strafing/sprint movement, native HUD, procedural
retro audio, a swaying weapon viewmodel, an active enemy AI that roams,
chases, melees and shoots back, jogged corridors with pillar-broken rooms,
`goto`-driven teleporter pads, timed spike traps and proximity mines at
corridor choke points, and a togglable automap. Reaching `return` doesn't end
the run: it carries your health and ammo into the next parsable file in the
tree, so a whole codebase plays as one continuous multi-level run.
