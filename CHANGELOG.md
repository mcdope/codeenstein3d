...
- Wall-edge antialiasing is back on by default: the perf audit measured its cost as undetectable on normal maps and ~0.4ms/frame on huge ones — smoother wall silhouettes at effectively no cost. Windowed-mode canvas resizing (filling the available window instead of staying a fixed size) stays off for now: its engine-side cost measured zero, but the browser-compositing side still needs one on-screen check before it ships
