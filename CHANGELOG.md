...
- Fix: loading a large GitHub repo (thousands of files) could keep fetching and parsing files well past what's needed to start playing — entrypoint detection now checks filenames before it needs any network content sniffing, an abandoned entrypoint scan actually stops once it's timed out instead of continuing in the background, and the whole-codebase highscore stats pass now skips itself for very large remote repos instead of trying to fetch and parse every file
